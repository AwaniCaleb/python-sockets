...
def main():
    ...
    try:
        ...
        # Determine the flood type based on arguments
        if args.threaded:
            flood_type = "spoofed" if args.spoof else "default"
            flooder.start_threaded(flood_type=flood_type, packet_count=args.count, spoofed_ip=args.spoofed_ip)
        else:
            if args.spoof:
